...
- 저장/조회가 필요한 application 서비스는 가능하면 domain 포트에 의존한다.
- Spring Data JPA 인터페이스는 infrastructure에 두고, 어댑터가 domain 포트를 구현한다.
...
| Domain Port | `*Store`, `*Port` | `UserStore`, `CouponIssuePort` |
| Adapter | `*Adapter` | `UserStoreJpaAdapter` |
...
| application → infrastructure 직접 의존 금지 | 저장소 구현체 대신 domain 포트에 의존 |

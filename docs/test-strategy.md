...
- 주문 상세/취소 경로의 `orderNumber` 형식 오류가 `400 + VALIDATION_ERROR`로 거절되는지 검증
...
- 저장소: 주문 요약 페이지 query가 `seller_order` 수를 projection으로 계산해 본인 주문만 반환하고, `createdAt` 동률에서도 `id` 보조 정렬로 안정적인 순서를 유지하는지 검증
...
- 단위: 쿠폰 정책 생성 규칙(금액/수량/기간)과 `READY/OPEN/CLOSED` 상태 전이 검증
- 단위: 쿠폰 발급 시 `OPEN` 상태, 발급 기간, 잔여 수량 검증
- 저장소: 쿠폰 정책 저장/조회, auditing 필드, `created_by` FK, 마이그레이션 제약 검증
- 저장소: `issued_coupon` 저장/중복 확인/사용자 보유 조회 정렬, `(coupon_policy_id, user_id)` 유니크 제약 검증
- 통합: 관리자 쿠폰 정책 생성/오픈/종료 API의 성공 계약, 역할 제한, CSRF 보호, 검증 실패, not found, 상태 전이 실패 검증
- 통합: USER 쿠폰 발급/보유 조회 API의 성공 계약, 역할 제한, CSRF 보호, 중복 발급 사전 차단 검증

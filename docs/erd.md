...
| --- | --- | --- |
| id | bigint | PK |
| order_number | varchar(40) | 고객용 공개 주문번호, unique |
| user_id | bigint | FK -> users.id |
...
제약:
- `order_number` unique (`uk_checkout_order_order_number`)

메모:
- 고객용 주문 조회/취소 API는 내부 PK 대신 `order_number`를 사용한다.
...
쿠폰 정책은 상품처럼 `deleted_at` soft delete를 두지 않고, 운영 종료를 `CLOSED` 상태 전이로 관리한다.
...
- `total_quantity >= 1`
- `issued_quantity >= 0`
- 한 번도 오픈하지 않은 정책도 운영 중단을 위해 `READY -> CLOSED` 전이를 허용한다.
...
메모:
- 동일 USER는 같은 `coupon_policy`를 중복 발급받을 수 없다.
- 보유 쿠폰 조회는 기본적으로 `ISSUED` 상태만 대상으로 한다.

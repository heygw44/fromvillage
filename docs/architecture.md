...
현재 구현은 장바구니 체크아웃과 바로 구매가 공통 주문 생성 컴포넌트를 공유하고, 진입점별 차이는 주문 대상 수집과 장바구니 삭제 여부로만 분리한다.
SELLER 주문 조회 API는 `seller_order` 단위로 별도 제공하며, 목록/상세 모두 구매자 정보는 닉네임까지만 노출한다.
고객용 주문 API는 `orderNumber`를 사용하고, SELLER 보호 조회 API는 `sellerOrderId`를 사용해 두 식별자 경계를 분리한다.
...
목록 정렬은 `createdAt`을 기본으로 하되, 같은 시각에 생성된 주문이 섞여도 페이지 경계가 흔들리지 않도록 `id`를 보조 정렬 키로 함께 사용한다.
...
현재 `M3-07` 단계까지는 쿠폰 연동 전이므로, 체크아웃과 바로 구매 모두 할인 금액 없이 주문을 생성하고 쿠폰 FK 연결은 후속 이슈에서 닫는다.

...
public enum ErrorCode {
    ...
    COUPON_POLICY_ISSUE_PERIOD_INVALID(HttpStatus.BAD_REQUEST, "COUPON_POLICY_ISSUE_PERIOD_INVALID", "쿠폰 발급 기간을 다시 확인해 주세요."),
    COUPON_POLICY_DISCOUNT_AMOUNT_INVALID(HttpStatus.BAD_REQUEST, "COUPON_POLICY_DISCOUNT_AMOUNT_INVALID", "할인 금액은 1원 이상이어야 합니다."),
    COUPON_POLICY_MINIMUM_ORDER_AMOUNT_INVALID(HttpStatus.BAD_REQUEST, "COUPON_POLICY_MINIMUM_ORDER_AMOUNT_INVALID", "최소 주문 금액은 0원 이상이어야 합니다."),
    COUPON_POLICY_TOTAL_QUANTITY_INVALID(HttpStatus.BAD_REQUEST, "COUPON_POLICY_TOTAL_QUANTITY_INVALID", "총 발급 수량은 1개 이상이어야 합니다."),
    COUPON_POLICY_STATUS_TRANSITION_INVALID(HttpStatus.CONFLICT, "COUPON_POLICY_STATUS_TRANSITION_INVALID", "변경할 수 없는 쿠폰 정책 상태입니다."),
    COUPON_ALREADY_ISSUED(HttpStatus.CONFLICT, "COUPON_ALREADY_ISSUED", "이미 발급받은 쿠폰입니다."),
    COUPON_POLICY_NOT_OPEN(HttpStatus.CONFLICT, "COUPON_POLICY_NOT_OPEN", "현재 발급할 수 없는 쿠폰입니다."),
    COUPON_POLICY_ISSUE_PERIOD_NOT_ACTIVE(HttpStatus.CONFLICT, "COUPON_POLICY_ISSUE_PERIOD_NOT_ACTIVE", "현재 발급 가능한 기간이 아닙니다."),
    COUPON_POLICY_QUANTITY_EXHAUSTED(HttpStatus.CONFLICT, "COUPON_POLICY_QUANTITY_EXHAUSTED", "쿠폰이 모두 소진되었습니다."),
